...
const LessonsTable = ({ el }) => {
  ...
  return (
    <tr>
      ...
      <td>
        <div className="d-none ">{(deleteId = el.id)}</div>
        <form
          method="GET"
          target="hiddenFrame"
          action={`https://cyf-finalproject-class-planner.herokuapp.com/api/lesson/delete/${deleteId}`}
        >
          <button
            type="submit"
            value="submit"
            ...
            className="nav_Home AdminRolesBTN mx-1"
          >
            Delete Class
          </button>
        </form>
